...
export async function downloadSource(value,date,{fetchOnce=curlOnce,sleep=ms=>new Promise(r=>setTimeout(r,ms)),waitMs=SOURCE_WAIT_MS,pollMs=SOURCE_POLL_MS,now=Date.now}={}){
 ...
 try{
  ...
  for(;;){
   ...
   if(now()+pollMs>deadline)throw Object.assign(problem(422,'PDF Ecostampa non ancora disponibile dopo '+checks+' controlli in '+Math.round(waitMs/1000)+' secondi'+(page.message?' (messaggio Ecostampa: «'+page.message+'»)':'')+'. Probabilmente è ancora in preparazione: la prossima importazione lo ricontrolla da sola, senza retry. Nessuna bozza creata.'),{sourceNotReady:true});
   await sleep(pollMs);
  }
