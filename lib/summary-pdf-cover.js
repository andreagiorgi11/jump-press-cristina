...
export async function drawEditionCover({pdf,page,body,font,bold,heading=bold,sectionFont=bold,selectedSections=null}){
 ...
 signals.forEach(signal=>{
  ...
  for(const line of paragraph){if(py<60){page=pdf.addPage([595.28,height]);py=height-60;}draw(line.text,textX+20,py,line.size,line.face,line.face===font?grey:ink);py-=12;}
  py-=13;
 });
}
