...
export async function drawEditionCover({pdf,page,body,font,bold,heading=bold,sectionFont=bold,selectedSections=null}){
 ...
 const logoMap={'Il Sole 24 Ore':'sole-24-ore','Il Sole 24 ORE':'sole-24-ore','Sole 24 Ore':'sole-24-ore','La Repubblica':'repubblica','Corriere dello Sport Stadio':'corriere-sport','Corriere dello Sport':'corriere-sport','La Gazzetta dello Sport':'gazzetta','La Stampa':'stampa','Tuttosport':'tuttosport'};
 ...
 for(const [i,outlet] of outlets.entries()){
  ...
  if(logoMap[outlet]){const logo=await pdf.embedPng(await readFile(process.cwd()+'/public/testate/pdf/'+logoMap[outlet]+'.png'));const factor=Math.min(115/logo.width,22/logo.height)*(outlet==='Tuttosport'?.9:1);page.drawImage(logo,{x:x+(150-logo.width*factor)/2,y:baseline+12-logo.height*factor/2,width:logo.width*factor,height:logo.height*factor});}
