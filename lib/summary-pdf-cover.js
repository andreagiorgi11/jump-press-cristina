...
export async function drawEditionCover({pdf,page,body,font,bold,heading=bold,selectedSections=null}){
 ...
 const fullHeight=Math.max(356,94+pointLines.reduce((n,l,i)=>n+l.length*14+18+(signals[i].groupLabel?25:0),0));
 ...
 const sentiment=dailySentiment(body.keyPoints),gx=margin+100,gy=top-291;
 ...
 centerText('Sentiment della giornata',top-238,9,bold);
 for(let i=0;i<36;i++){
 ...
 centerText(sentiment.label,gy-29,8,bold,ink);centerText('Dai punti chiave della rassegna',gy-42,6.5);
 if(separatePoints){
