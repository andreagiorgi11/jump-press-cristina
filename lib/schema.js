...
export const executiveSummarySchema=z.object({intro:text.min(1).max(1200),sections:z.tuple(summaryAreas.map((title,i)=>z.object({title:z.literal(title),items:z.array(text.min(1).max(500)).max(i===0?5:30)}).strict()))}).strict();
