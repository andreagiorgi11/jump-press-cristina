import {summarySections,legacySummarySections,articleSections} from './summary-sections.js';
...
export const articleSchema=z.object({
  ...
  juventusSentiment:z.object({tone:z.enum(['positivo','negativo','neutro','misto']),reason:text.min(1).max(600)}).strict().nullable().optional().describe('Obsoleto: non compilare. Il server lo rimuove al salvataggio.'),
...
export const editionSchema=z.object({
  ...
  executiveSummaryStale:z.boolean().optional().describe('Impostato dal server: il Summary va ricontrollato dopo modifiche sostanziali. Non compilare.'),
...
export const mcpSummaryEditionSchema=editionSchema.safeExtend({
 articles:z.array(articleSchema.extend({category:z.enum(articleSections),isEditorial:z.boolean().describe('Obbligatorio: true per editoriali, false per notizie e interviste.')})).max(80),
