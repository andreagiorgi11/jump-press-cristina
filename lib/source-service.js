...
import {blobs,readSourcePages,writeSourceClip} from './blob-store.js';
...
export async function readImportPage(ctx,id,page){const row=await getImport(ctx,id);if(row.status!=='ready'||row.originalDeletedAt)throw problem(410,'Originale non disponibile per la lettura visiva.');return renderSourcePage(await readSourcePages(storage(ctx),row.originalPath,[page]),1);}
export async function createImportClip(ctx,{importId,draftId,pages}){
 ...
 // Cut next to the original: only the clip travels, never the whole source.
 await writeSourceClip(storage(ctx),row.originalPath,pages,clip.storage_path);
...
export async function readImportPages(ctx,id,pages){
 ...
 // Only the requested pages are fetched; page numbers are mapped back to the source numbering.
 const result=await renderSourcePages(await readSourcePages(storage(ctx),row.originalPath,pages),pages.map((_,i)=>i+1));
 return {images:result.images.map(i=>({...i,page:pages[i.page-1]})),remainingPages:result.remainingPages.map(n=>pages[n-1])};
}
export async function createImportClips(ctx,{importId,draftId,version,items}){
 ...
 // Stores that cut pages themselves need nothing; others read the original once for the whole batch.
 const s=storage(ctx);let original;const batchCtx=s.clip?ctx:{...ctx,blobs:{...s,readOriginal:p=>original??=s.readOriginal(p)}},completed=[];
