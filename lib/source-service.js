...
import {extractSourceText,renderSourcePage} from './source-pdf.js';
...
export function importStatus(row){return {importId:row.id,date:row.date,name:row.name,status:row.status,pageCount:row.pageCount,bytes:row.bytes,sha256:row.sha256,totalCharacters:row.totalCharacters,pagesWithLittleText:row.pagesWithLittleText,error:row.error,originalDeletedAt:row.originalDeletedAt,note:'Testi estratti automaticamente, non verificati editorialmente. Le pagine senza testo richiedono lettura visiva; nessun OCR automatico. Leggere tutte le pagine prima di dichiarare copertura completa.'};}
...
export async function importPdf(ctx,{url,date,retry=false}){
 ...
 if(existing){const prior=await r.read(record(existing.id),head);if(!prior)throw problem(503,'Indice importazioni non coerente.');
  if(prior.status==='ready'||!retry)return importStatus(prior);
 ...
 const work=async()=>{try{
  ...
  row.bytes=bytes.length;row.sha256=createHash('sha256').update(bytes).digest('hex');row.pageCount=text.pageCount;row.totalCharacters=text.totalCharacters;row.pagesWithLittleText=text.pagesWithLittleText;
  ...
  row.status='failed';row.error=error.status&&error.status<500?error.message:'Preparazione della fonte non riuscita. Verifica lo stato prima di riprovare.';
...
export async function createImportClip(ctx,{importId,draftId,pages}){
 ...
 const input=await PDFDocument.load(await storage(ctx).readOriginal(row.originalPath)),out=await PDFDocument.create();
 ...
 if(!source)source={id:randomUUID(),draft_id:draftId,kind:'source_reference',importId,name:row.name,date:row.date,sha256:row.sha256,pageCount:row.pageCount};
...
export async function retirementFiles(r,head,d,now=Date.now()){
 ...
 for(const id of ids){const row=await r.read(record(id),head);if(!row)throw problem(503,'Fonte importata non trovata.');if(row.deleteAfter||row.originalDeletedAt)continue;
  row.deleteAfter=new Date(now+7*24*60*60*1000).toISOString();files[record(id)]=row;queue.items.push({id,deleteAfter:row.deleteAfter});}
