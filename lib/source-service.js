...
export function importStatus(row){return {importId:row.id,date:row.date,name:row.name,status:row.status,pageCount:row.pageCount,bytes:row.bytes,sha256:row.sha256,totalCharacters:row.totalCharacters,pagesWithLittleText:row.pagesWithLittleText,error:row.error,startedAt:row.startedAt,completedAt:row.completedAt,timings:row.timings||null,originalDeletedAt:row.originalDeletedAt,note:'Testi estratti automaticamente, non verificati editorialmente. Le pagine senza testo richiedono lettura visiva; nessun OCR automatico. Leggere tutte le pagine prima di dichiarare copertura completa.'};}
