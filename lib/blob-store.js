...
const putFile=(path,bytes,type,timeout=120000)=>call('/o/'+encodePath(path),{method:'PUT',body:bytes,headers:{'Content-Type':type},timeout});
const serverBlobs={
 async writeOriginal(path,bytes){if(bytes.length>200*1024*1024)throw problem(413,'Originale oltre 200 MB.');await putFile(path,bytes,'application/pdf',240000);},
