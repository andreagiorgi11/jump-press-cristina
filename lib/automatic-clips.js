...
// Headline type is often garbled by text extraction (lost first letter, cut last word).
// Partial title support only counts together with a literal article quote on the same page.
function titleWordsOnPage(title,text){
 const page=normalizeTitle(text),words=[...new Set((String(title).normalize('NFKD').replace(/[̀-ͯ]/g,'').toLowerCase().match(/[a-z0-9]+/g)||[]).filter(w=>w.length>=4).map(normalizeTitle))];
 const found=words.filter(w=>page.includes(w)).length;
 return words.length>0&&found>=Math.min(2,words.length)&&found/words.length>=1/3;
}
export function verifyArticle(a,pages){
 ...
 const quoteOnFirst=!!first&&evidence.some(e=>e.matchedPages.includes(first.page));
 const pdf=first&&titleMatches(a.title,first.text)?{status:'matched',note:'Titolo riconosciuto sulla prima pagina indicata.'}
  :quoteOnFirst&&titleWordsOnPage(a.title,first.text)?{status:'matched',note:'Titolo letto solo in parte nel testo estratto dal PDF; estratti dell’articolo riscontrati sulla prima pagina indicata.'}
  :{status:'attention',note:'Titolo non riconosciuto con sicurezza sulla prima pagina indicata.'};
...
export async function prepareAutomaticClips(ctx,draft,previous){
 ...
 const priorArticle=a=>prior?.body.articles.find(p=>p.id===a.id);
 // Only settled checks are reused; warnings are recomputed on each save instead of being copied forward.
 const reusable=a=>{const p=priorArticle(a);return p&&p.clipId&&p.pdfCheck&&p.synthesisCheck&&p.pdfCheck.status==='matched'&&p.synthesisCheck.status==='verified'&&articleFingerprint(p)===articleFingerprint(a)&&isDeepStrictEqual(p.factCheck,a.factCheck);};
 ...
 try{
  ...
  if(body.articles.some(a=>!reusable(a))){
   for(let attempt=0;;attempt++){
    ...
    catch(error){if(attempt===2||error.message==='inconsistent source')throw error;console.warn('[Jump Press] source text retry',attempt+1,String(error.message||error).slice(0,200));await new Promise(resolve=>setTimeout(resolve,700*(attempt+1)));}
   }
  }
  ...
  for(const a of body.articles){
   ...
   a.synthesisCheck=reusable(a)?p.synthesisCheck:{status:'pending',sourceStatus:'incomplete',note:'Fonte temporaneamente non disponibile per verificare gli estratti.'};
  }
 }
 async function clipFor(pages){
  ...
  if(clip){
   const known=prior?.assets?.some(a=>a.id===clip.id&&a.storage_path===clip.storage_path)&&(
    prior.body.articles.some(a=>a.clipId===clip.id&&isDeepStrictEqual(a.pages,pages))||prior.body.coverage?.frontPages?.some(p=>p.clipId===clip.id&&isDeepStrictEqual([p.page],pages)));
  ...
  await writeSourceClip(storage,sourceRow.originalPath,pages,clip.storage_path);assets.push(clip);files['assets/'+id+'.json']=clip;return clip;
 }
 if(source){
  for(const a of body.articles){
   if(reusable(a)){const p=priorArticle(a);a.pdfCheck=p.pdfCheck;a.synthesisCheck=p.synthesisCheck;}else{const checks=verifyArticle(a,text.pages);a.pdfCheck=checks.pdf;a.synthesisCheck=checks.synthesis;}
 ...
 const next={...current,body,assets,version:current.version+1,updated_at,automaticClips:{sourceUnavailable,status:sourceUnavailable||body.articles.some(a=>a.pdfCheck.status!=='matched')||(body.coverage?.frontPages||[]).some(p=>p.juventus&&!p.clipId)?'attention':'complete',durationMs:Date.now()-started,checkedAt:updated_at},revisions:[{version:current.version+1,created_at:updated_at},...(current.revisions||[])].slice(0,50)};
