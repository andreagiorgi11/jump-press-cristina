...
export function verifyArticle(a,pages){
 ...
 const issues=[];
 if(!check?.evidence?.length)issues.push('Confronto fonte–sintesi non documentato: mancano gli estratti.');
 for(const [index,e] of (check?.evidence||[]).entries()){
  const label='Estratto '+(index+1),quote=normalizeSource(e.quote),page=pages.find(p=>p.page===e.page);
  if(!a.pages.includes(e.page)){issues.push(label+': pagina '+e.page+' non assegnata all’articolo.');continue;}
  if(quote.length<15){issues.push(label+': troppo breve per un riscontro affidabile.');continue;}
  if(!page||typeof page.text!=='string'||!page.text.trim()){issues.push(label+': testo della pagina '+e.page+' non disponibile per il controllo.');continue;}
  if(normalizeSource(page.text).includes(quote))continue;
  const elsewhere=pages.filter(p=>a.pages.includes(p.page)&&p.page!==e.page&&normalizeSource(p.text).includes(quote)).map(p=>p.page);
  issues.push(elsewhere.length?label+': trovato nelle pagine '+elsewhere.join(', ')+', non nella pagina '+e.page+' indicata; verificare il riferimento.':label+': non riscontrato letteralmente nella pagina '+e.page+'; verificare citazione e testo estratto dal PDF.');
 }
 if(check?.status!=='verified')issues.push(check?.note?'Verifica editoriale da completare: '+check.note:'Verifica editoriale non dichiarata completa da GPT.');
 const synthesis=issues.length?{status:'attention',note:issues.join(' ').slice(0,1000)}:{status:'verified',note:'Confronto fattuale dichiarato da GPT; estratti riscontrati nella fonte. Non è una certificazione automatica del significato.'};
