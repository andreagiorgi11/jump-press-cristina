...
export function verifyArticle(a,pages){
 ...
 // Large display headlines often come out broken in the extracted text: literal quotes found on the
 // first assigned page confirm the clip, so an unrecognised title alone is not a PDF warning.
 if(pdf.status!=='matched'&&evidence.some(e=>e.matchedPages.includes(a.pages[0])))Object.assign(pdf,{status:'matched',note:'Pagina confermata dagli estratti; titolo non leggibile nel testo estratto.'});
...
export async function prepareAutomaticClips(ctx,draft,previous){
 ...
 let sourceRow,text,input,source,sourceUnavailable=false;
 try{
 ...
 async function clipFor(pages){
  ...
  if(!input){if(sourceRow.originalDeletedAt)throw Error('original unavailable');input=await PDFDocument.load(await storage.readOriginal(sourceRow.originalPath));if(input.getPageCount()!==sourceRow.pageCount)throw Error('inconsistent source');}
