...
export async function prepareAutomaticClips(ctx,draft,previous){
 ...
 const reusable=a=>{const p=priorArticle(a);return p&&p.clipId&&p.pdfCheck&&p.synthesisCheck&&p.pdfCheck.status!=='pending'&&p.synthesisCheck.status!=='pending'&&articleFingerprint(p)===articleFingerprint(a)&&isDeepStrictEqual(p.factCheck,a.factCheck);};
 ...
 try{
  ...
  if(body.articles.some(a=>!reusable(a))){text=await storage.readText(sourceRow.textPath);if(!Array.isArray(text.pages))throw Error('inconsistent source');}
