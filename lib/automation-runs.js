...
function summary(row){return {status:row.status,date:row.date,draftId:row.draftId,generation:row.generation,leaseUntil:row.leaseUntil,phase:row.phase,checkpoint:row.checkpoint,timings:row.timings,startedAt:row.startedAt,updatedAt:row.updatedAt,draftVersion:row.draftVersion,retryable:row.retryable,attemptsRemaining:MAX_ATTEMPTS-row.generation};}
...
export async function claimRun(ctx,{date,url,requestId,resume=false}){
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  if(prior&&(!resume||!prior.retryable||prior.generation>=MAX_ATTEMPTS))return {...summary(prior),acquired:false,reason:'recovery_requires_review',canResume:prior.retryable&&prior.generation<MAX_ATTEMPTS};
...
export async function updateRun(ctx,{run,phase,checkpoint={},status='running',retryable=false,draftVersion}){
 ...
 if(status==='completed'){
  ...
  const body=editionSchema.parse(draft.body);await validateSummaryReady(body);
