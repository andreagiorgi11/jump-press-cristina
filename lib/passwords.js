...
export async function authenticate(username,password,repo=store){
 ...
 username=username.trim().toLowerCase();await reserveLogin(username,repo);
 const u=list.find(u=>u.username===username);
 if(!await passwordMatches(password,u?.passwordHash))throw problem(401,'Nome utente o password non corretti.');
