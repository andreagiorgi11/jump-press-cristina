...
export function roleFor(id){let members;try{members=JSON.parse(process.env.JUMP_GITHUB_MEMBERS||'{}');}catch{throw problem(503,'Elenco editor non configurato correttamente.');}const role=members[String(id)];if(!['producer','editor','publisher'].includes(role))throw problem(403,'Account GitHub non abilitato alla redazione.');return role;}
...
export async function readAccess(token,repo=store){const p=await verify(token,'mcp-access');const role=roleFor(p.sub);const grant=await repo.read('oauth/grants/'+p.sid+'.json',await repo.begin());if(!grant||grant.revoked||grant.expires<Date.now()||grant.user.id!==p.sub)throw problem(401,'Collegamento revocato o scaduto.');return {user:grant.user,role:capRole(role,grant.role)};}
export async function consumeCode(code,clientId,redirectUri,verifier,repo=store){
 ...
 const refreshId=nonce(),grant={user:p.user,role:capRole(roleFor(p.user.id),p.role),clientId,refreshHash:hash(refreshId),expires:Date.now()+30*86400000,revoked:false};
...
export async function refreshGrant(token,clientId,repo=store){
 ...
 grant.role=capRole(roleFor(grant.user.id),grant.role);const refreshId=nonce();grant.refreshHash=hash(refreshId);
