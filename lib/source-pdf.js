import {extractSourceOutlets} from './source-outlets.js';
...
export async function extractSourceText(bytes){
 ...
 try{
  ...
  return {sourceOutlets:extractSourceOutlets(pages),pageCount:doc.numPages,totalCharacters,pagesWithLittleText:pages.filter(p=>p.needsVisualCheck).map(p=>p.page),pages};
