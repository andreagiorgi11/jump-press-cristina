...
import {areaColor} from './key-point-signals.js';
import {withNationalSection} from './summary-sections.js';
...
export function analyseEdition(body,assets=[]){
  // Same grouping as the page: Nazionale is its own theme only from four articles.
  const articles=withNationalSection(body.articles||[]),counts=new Map();
  ...
  const themes=[...counts].map(([label,count],i)=>{
    ...
    return {label,count,percent:count/articles.length*100,color:areaColor(label,i),start,end:cursor};
  });
