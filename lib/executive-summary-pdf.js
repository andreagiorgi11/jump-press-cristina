...
export async function exportSummaryPdf(summary,date){
 ...
 for(const [index,section] of summary.sections.entries()){
  ...
  paragraph(`${String(index+1).padStart(2,'0')}  ${section.title.replace('Temi varie','Temi vari')}`,{size:11,strong:true,gap:7});
  for(const item of section.items){page.drawCircle({x:left+3,y:y+3,size:1.6,color:grey});paragraph(item,{x:left+13,max:width-13,gap:5});}
 }
