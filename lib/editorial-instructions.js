...
export async function readInstructions(ctx){access(ctx);const repo=ctx.store||store;const head=await repo.begin(),current=await localPreview(ctx,usesSummaryModel(ctx)?await profileAt(repo,head):await at(repo,head));return {...current,...(usesSummaryModel(ctx)?{editorialModel:'summary-v1',instructionProfile:'summary-v1'}:{}),automationPrompt,scheduledPublicationAllowed:false};}
