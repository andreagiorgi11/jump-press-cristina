...
export async function readInstructions(ctx){access(ctx);const repo=ctx.store||store;const head=await repo.begin(),current=usesSummaryModel(ctx)?await profileAt(repo,head):await localPreview(ctx,await at(repo,head));return {...current,...(usesSummaryModel(ctx)?{editorialModel:'summary-v1',instructionProfile:'summary-v1'}:{}),automationPrompt,scheduledPublicationAllowed:false};}
export async function saveInstructions(ctx,version,text){
 ...
 const summary=usesSummaryModel(ctx),target=summary?summaryPath:path;
 const repo=ctx.store||store,head=await repo.begin(),current=summary?await profileAt(repo,head):await at(repo,head);
 if(!summary&&(await localPreview(ctx,current)).localPreview)throw problem(409,'Anteprima locale attiva: salvataggio online delle istruzioni disabilitato in questo ambiente.');
 ...
 await repo.commit({[target]:next,['settings/'+(summary?'summary-':'')+'editorial-history/'+next.version+'.json']:next},head,'Aggiornamento istruzioni rassegna');
