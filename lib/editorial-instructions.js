import {summaryEditorialText} from './summary-instructions.js';
...
export async function readInstructions(ctx){access(ctx);const repo=ctx.store||store;const current=await localPreview(ctx,await at(repo,await repo.begin()));return {...current,...(usesSummaryModel(ctx)?{text:summaryEditorialText,editorialModel:'summary-v1',instructionProfile:'summary-v1-local',baseVersion:current.version}:{}),automationPrompt,scheduledPublicationAllowed:false};}
export async function saveInstructions(ctx,version,text){
 ...
 if(usesSummaryModel(ctx))throw problem(409,'Profilo Summary sperimentale: le istruzioni sono versionate nel progetto separato; salvataggio online disabilitato.');
