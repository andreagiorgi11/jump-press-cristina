import {originalPageCount,totalOutletCount} from './coverage-stats.js';
import {isDeepStrictEqual} from 'node:util';
import {prepareAutomaticClips,articleFingerprint} from './automatic-clips.js';
import {cleanSummary} from './article-author.js';
import {usesSummaryModel,validateSummaryReady,editorialContent,summarySubstance} from './summary-workflow.js';
...
// MCP returns an explicit tombstone only for an editor-authorized relaunch archive.
export async function readDraftForAutomation(ctx,id){
 editor(ctx);const r=repo(ctx),head=await r.begin(),d=await r.read(path(id),head);
 if(d?.deletedAt&&d.archivedAt&&d.archivePurpose==='relaunch'){
  const index=await readIndex(r,head);
  const listed=[...index.drafts,...(index.trash||[])].some(x=>x.id===id)||index.published.some(x=>x.draft_id===id);
  if(!listed)return {id,date:d.body.date,status:'archived_for_relaunch',active:false,message:'Questa bozza è stata archiviata dall’editor per una nuova elaborazione e non costituisce un duplicato attivo. Non ripristinarla e non riusarne l’ID. Rileggi lo stato della data e usa claim_automation_run: prosegui soltanto se acquired=true con il nuovo draftId assegnato. Eventuali altre bozze, pubblicazioni o esecuzioni attive restano protette.'};
 }
 return draftAt(ctx,id,head);
}
// Partial saves update articles instead of replacing them: a field omitted by the client keeps its saved value.
// Fields are kept only while it is evidently the same piece (same outlet and same first page): a different piece under
// the same id inherits nothing. An explicit value, including null, always wins; to replace a piece clients use a new id.
const keptFields=['isEditorial','showAuthor','author','rating','topic','factCheck','pages','sourceId','clipId'];
function omittedFields(input){const out=new Map();for(const a of Array.isArray(input?.articles)?input.articles:[])if(a&&typeof a==='object')out.set(a.id,new Set(keptFields.filter(k=>!(k in a))));return out;}
function keepOmitted(body,old,omitted){
 for(const a of body.articles){
  const previous=old.body.articles.find(p=>p.id===a.id),missing=omitted.get(a.id);if(!previous||!missing)continue;
  const samePiece=previous.outlet===a.outlet&&(missing.has('pages')||previous.pages?.[0]===a.pages?.[0]);
  if(samePiece)for(const key of keptFields)if(missing.has(key)&&previous[key]!==undefined)a[key]=structuredClone(previous[key]);
 }
}
export async function saveDraft(ctx,id,version,input,{summaryConfirmed=false}={}){
 editor(ctx);const omitted=omittedFields(input),body=editionSchema.parse(input);delete body.executiveSummaryStale;
 // The editorial label and byline are rendered by site and PDF, never repeated in the summary.
 for(const a of body.articles)a.summary=cleanSummary(a);const r=repo(ctx),head=await r.begin(),index=await readIndex(r,head);
 ...
 if(!body.sourceImportId&&old?.body.sourceImportId)body.sourceImportId=old.body.sourceImportId;
 if(!body.sourceImportId&&usesSummaryModel(ctx)&&ctx.automation){const run=await r.read('automation/runs/'+ctx.automation.date+'.json',head);if(run?.checkpoint?.importId)body.sourceImportId=run.checkpoint.importId;}
 if(body.sourceImportId&&ctx.automation){const run=await r.read('automation/runs/'+ctx.automation.date+'.json',head);if(run?.checkpoint?.importId!==body.sourceImportId)throw problem(409,'Usare la fonte assegnata al lavoro corrente.');}
 if(old)keepOmitted(body,old,omitted);
 // Per-article sentiment is retired (24/09/2026): accepted for old drafts, never stored again.
 for(const article of body.articles)delete article.juventusSentiment;
 if(usesSummaryModel(ctx)&&!old)body.editorialModel='summary-v1';
 // The Summary is never deleted by an edit. Substantive changes mark it to be rechecked; a new Summary or an
 // explicit confirmation makes it current again. Form-only edits leave its state unchanged.
 if(old?.body.editorialModel==='summary-v1'){body.editorialModel='summary-v1';if(body.executiveSummary===undefined)body.executiveSummary=old.body.executiveSummary??null;
  const renewed=JSON.stringify(body.executiveSummary)!==JSON.stringify(old.body.executiveSummary)||summaryConfirmed;
  if(body.executiveSummary&&!renewed&&(old.body.executiveSummaryStale||summarySubstance(old.body)!==summarySubstance(body)))body.executiveSummaryStale=true;}
 ...
 // Removing a repeated editorial label is not an edit: an otherwise identical save re-runs the checks.
 const cleanedOld=old&&{...old.body,articles:old.body.articles.map(a=>({...a,summary:cleanSummary(a)}))};
 const reusePreparation=!!(body.sourceImportId&&old?.automaticClips&&(old.automaticClips.status==='complete'||!isDeepStrictEqual(cleanedOld,body))&&old.body.sourceImportId===body.sourceImportId&&old.body.date===body.date&&isDeepStrictEqual(old.body.coverage,body.coverage)&&body.articles.length===old.body.articles.length&&body.articles.every(a=>{const p=old.body.articles.find(p=>p.id===a.id);return p&&p.clipId&&p.pdfCheck&&p.synthesisCheck&&p.pdfCheck.status!=='pending'&&p.synthesisCheck.status!=='pending'&&articleFingerprint(p)===articleFingerprint(a)&&isDeepStrictEqual(p.factCheck,a.factCheck);}));
 if(body.sourceImportId){
  for(const a of body.articles){
   const prior=old?.body.articles.find(p=>p.id===a.id);
   if(prior&&(old.body.sourceImportId!==body.sourceImportId||articleFingerprint(prior)!==articleFingerprint(a))&&isDeepStrictEqual(prior.factCheck,a.factCheck))a.factCheck=null;
   // These statuses are server-owned. Client-supplied successes cannot bypass checks.
   a.pdfCheck={status:'pending',note:'Preparazione PDF in corso; se interrotta, riprovare il salvataggio.'};
   a.synthesisCheck={status:'attention',note:'Sintesi da verificare: confronto con la fonte non ancora riscontrato.'};
   const retained=old?.body.sourceImportId===body.sourceImportId&&prior&&isDeepStrictEqual(prior.pages,a.pages);
   a.clipId=retained?prior.clipId:null;a.sourceId=retained?prior.sourceId:null;
   if(reusePreparation){a.pdfCheck=prior.pdfCheck;a.synthesisCheck=prior.synthesisCheck;}
  }
 }
 if(old&&isDeepStrictEqual(old.body,body)&&!body.sourceImportId)return old;
 ...
 await r.commit({[path(id)]:next,[revisionPath(id,nextVersion)]:{body,actor:ctx.user.id,created_at:updated_at},'index.json':index},head,'Salvataggio bozza Jump Press');
 if(body.sourceImportId&&!reusePreparation)return prepareAutomaticClips(ctx,next,old);
 return next;
}
...
export function publicBody(body,assets=[]){return {sourceOutletCount:totalOutletCount(body,assets),sourcePageCount:originalPageCount(body,assets),...(body.editorialModel?{editorialModel:body.editorialModel,executiveSummary:body.executiveSummary??null}:{}),date:body.date,title:body.title,intro:body.intro,coverage:body.coverage?{examinedItems:body.coverage.examinedItems,frontPages:body.coverage.frontPages?.map(({sourceId,...page})=>page)??null,frontPageSummary:body.coverage.frontPageSummary}:null,toneSummary:body.toneSummary||'',keyPoints:body.keyPoints,tones:body.tones,metrics:body.metrics.map(({label,value})=>({label,value})),articles:body.articles.map(({id,category,topic,title,outlet,author,isEditorial,showAuthor,summary,rating,clipId,juventusSentiment})=>({id,category,...(topic?{topic}:{}),title,outlet,author,...(isEditorial!==undefined?{isEditorial}:{}),...(showAuthor!==undefined?{showAuthor}:{}),summary,rating,clipId,...(juventusSentiment?{juventusSentiment}:{})}))};}
...
export async function publishDraft(ctx,id,version,confirmation){
 ...
 const row={edition_date:date,draft_id:id,version,published_at:new Date().toISOString()},published={...row,body:publicBody(d.body,d.assets)};
