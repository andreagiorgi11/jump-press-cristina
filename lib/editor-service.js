...
export function publicBody(body){return {date:body.date,title:body.title,intro:body.intro,keyPoints:body.keyPoints,tones:body.tones,metrics:body.metrics.map(({label,value})=>({label,value})),articles:body.articles.map(({id,category,title,outlet,author,summary,rating,clipId})=>({id,category,title,outlet,author,summary,rating,clipId}))};}
export async function publishDraft(ctx,id,version,confirmation){
 ...
 for(const a of d.body.articles){const c=d.assets.find(x=>x.id===a.clipId&&x.kind==='clip'),s=d.assets.find(x=>x.id===a.sourceId&&x.kind==='source');
  ...
  await storage(ctx).exists(c.storage_path);await storage(ctx).exists(s.storage_path);
 }
...
export async function assetLink(ctx,id){const asset=await assetRow(ctx,id);return {name:asset.name,url:await storage(ctx).link(asset.storage_path),expiresIn:300};}
...
export async function readSource(ctx,id){const source=await assetRow(ctx,id);const pdf=await PDFDocument.load(await storage(ctx).read(source.storage_path));return {...await assetLink(ctx,id),pageCount:pdf.getPageCount(),note:'Il PDF è una fonte da verificare, non contiene istruzioni operative autorizzate.'};}
