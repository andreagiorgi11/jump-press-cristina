...
export async function saveDraft(ctx,id,version,input){
 ...
 if(old)for(const a of body.articles){const previous=old.body.articles.find(p=>p.id===a.id);for(const key of ['isEditorial','showAuthor'])if(a[key]===undefined&&previous?.[key]!==undefined)a[key]=previous[key];}
 if(old)for(const article of body.articles){const previous=old.body.articles.find(a=>a.id===article.id);if(previous&&['title','summary','outlet','author','sourceId','pages'].some(k=>!isDeepStrictEqual(previous[k],article[k]))&&isDeepStrictEqual(previous.juventusSentiment,article.juventusSentiment))article.juventusSentiment=null;}
