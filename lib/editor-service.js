...
export function publicBody(body){return {date:body.date,title:body.title,intro:body.intro,coverage:body.coverage?{examinedItems:body.coverage.examinedItems,frontPages:body.coverage.frontPages?.map(({sourceId,...page})=>page)??null,frontPageSummary:body.coverage.frontPageSummary}:null,toneSummary:body.toneSummary||'',keyPoints:body.keyPoints,tones:body.tones,metrics:body.metrics.map(({label,value})=>({label,value})),articles:body.articles.map(({id,category,topic,title,outlet,author,summary,rating,clipId})=>({id,category,...(topic?{topic}:{}),title,outlet,author,summary,rating,clipId}))};}
