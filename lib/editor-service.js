...
export async function saveDraft(ctx,id,version,input,{summaryConfirmed=false}={}){
 editor(ctx);const omitted=omittedFields(input),body=editionSchema.parse(input);delete body.executiveSummaryStale;const r=repo(ctx),head=await r.begin(),index=await readIndex(r,head);
 ...
 const reusePreparation=!!(body.sourceImportId&&old?.automaticClips&&(old.automaticClips.status==='complete'||!isDeepStrictEqual(old.body,body))&&old.body.sourceImportId===body.sourceImportId&&old.body.date===body.date&&isDeepStrictEqual(old.body.coverage,body.coverage)&&body.articles.length===old.body.articles.length&&body.articles.every(a=>{const p=old.body.articles.find(p=>p.id===a.id);return p&&p.clipId&&p.pdfCheck&&p.synthesisCheck&&p.pdfCheck.status!=='pending'&&p.synthesisCheck.status!=='pending'&&articleFingerprint(p)===articleFingerprint(a)&&isDeepStrictEqual(p.factCheck,a.factCheck);}));
