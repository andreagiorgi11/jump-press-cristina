import {showAuthor} from './article-author.js';
...
export async function exportEditionPdf(source,fonts,selectedSections=null,{loadClip}={}){
 ...
 await drawEditionCover({pdf,page,body:coverBody,font,bold,heading:serif,sectionFont:sectionFont||bold,selectedSections:partial?summarySections.filter(s=>selectedSections.includes(s)):null});next();
 ...
 const articleRows=article=>{
  ...
  if(article.author&&showAuthor(article))add('di '+article.author,8.5,font,grey,8);
