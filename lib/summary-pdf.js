...
export async function exportEditionPdf(source,fonts,selectedSections=null,{loadClip}={}){
 const body=summaryEdition(source);body.articles.sort((a,b)=>articleSections.indexOf(a.category)-articleSections.indexOf(b.category));
 ...
 await drawEditionCover({pdf,page,body:coverBody,font,bold,heading:serif,sectionFont:sectionFont||bold,selectedSections:partial?articleSections.filter(s=>selectedSections.includes(s)):null});next();
