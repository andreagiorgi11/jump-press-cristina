...
export async function exportEditionPdf(source){
 ...
 const font=await pdf.embedFont(await readFile(process.cwd()+'/.local/arial.ttf'),{subset:true}),bold=await pdf.embedFont(await readFile(process.cwd()+'/.local/arialbd.ttf'),{subset:true}),serif=bold;
