...
export default function PdfSectionPicker({onClose,onConfirm,onSummary,summaryAvailable=true,initialSections=null,initialClips=false,onChange}){
 const dialog=useRef(null),[selected,setSelected]=useState(initialSections??articleSections.map((_,i)=>i));
 ...
 const all=selected.length===articleSections.length;
 ...
 return <dialog ref={dialog} className="pdf-section-picker" aria-labelledby="pdf-picker-title" onCancel={onClose}>
  ...
  <label className="pdf-pick-all"><input type="checkbox" checked={all} onChange={()=>setSelected(all?[]:articleSections.map((_,i)=>i))}/><strong>Tutta la rassegna</strong></label>
  <fieldset><legend>Sezioni incluse</legend>{articleSections.map((name,i)=><label key={name}><input type="checkbox" checked={selected.includes(i)} onChange={()=>toggle(i)}/><span>{name}</span></label>)}</fieldset>
