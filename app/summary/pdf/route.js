...
export async function GET(request){
 ...
 const bytes=summary?await exportSummaryPdf(source.editorialModel==='summary-v1'?source.executiveSummary:JSON.parse(await readFile(process.cwd()+'/.local/summary-preview.json','utf8')),source.date,{regular:await readFile(process.cwd()+'/.local/arial.ttf'),bold:await readFile(process.cwd()+'/.local/arialbd.ttf')}):await exportEditionPdf(source);
