...
export default function ExportPage({date,live=false,pdfEndpoint,summaryAvailable=true,summaryAlert=null,coverageStats,editorPanel=null}){
 ...
 <aside className={`summary-sidebar${menuOpen?' is-open':''}`} onKeyDown={event=>{if(event.key==='Escape'&&menuOpen){event.preventDefault();closeMenu();}}}>
  <div className="summary-sidebar-heading"><a className="sidebar-client-mark" href="#rassegna-oggi" aria-label="Juventus, rassegna di oggi" onClick={()=>setMenuOpen(false)}><img src="/brand/juventus-j.svg" alt="Juventus" width="33" height="52"/></a><button ref={toggle} className={'summary-menu-toggle'+(editorPanel?.attention?' has-attention':'')} type="button" aria-expanded={menuOpen} aria-controls="summary-sidebar-menu" aria-label={menuOpen?'Chiudi menu':'Apri menu'} onClick={()=>setMenuOpen(!menuOpen)}><Icon kind={menuOpen?'close':'menu'}/></button></div>
