...
export default function ExportPage({date,live=false,pdfEndpoint,summaryAvailable=true,summaryAlert=null,coverageStats}){
 ...
 <aside className={`summary-sidebar${menuOpen?' is-open':''}`} onKeyDown={event=>{if(event.key==='Escape'&&menuOpen){event.preventDefault();closeMenu();}}}>
  <div className="summary-sidebar-heading"><a className="sidebar-client-mark" href="#rassegna-oggi" aria-label="Juventus, rassegna di oggi" onClick={()=>setMenuOpen(false)}><img src="/brand/juventus-j.svg" alt="Juventus" width="33" height="52"/></a><button ref={toggle} className="summary-menu-toggle" type="button" aria-expanded={menuOpen} aria-controls="summary-sidebar-menu" aria-label={menuOpen?'Chiudi menu':'Apri menu'} onClick={()=>setMenuOpen(!menuOpen)}><Icon kind={menuOpen?'close':'menu'}/></button></div>
  <div className="summary-sidebar-menu" id="summary-sidebar-menu">

   <nav aria-label="Navigazione rassegna"><div className="summary-nav-heading"><p className="summary-nav-label">LEGGI</p><button className="sidebar-refresh" type="button" title="Aggiorna rassegna" aria-label="Aggiorna rassegna" aria-busy={refreshing} disabled={refreshing} onClick={refresh}><Icon kind="refresh"/></button></div><a className="summary-nav-item is-active" href="#rassegna-oggi" aria-current="page" onClick={()=>setMenuOpen(false)}><Icon kind="today"/><span>Rassegna di oggi</span></a>
   ...
   <button type="button" className="summary-nav-item" aria-haspopup="dialog" onClick={event=>openPdf('edition',event)}><Icon kind="download"/><span>Scarica PDF</span>{summaryAlert&&<span className="summary-pdf-alert" role="img" aria-label={summaryAlert} title={summaryAlert}><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M12 3.2 22 20.5H2L12 3.2Z" fill="#f2a516" stroke="#c77c15" strokeWidth="1.2" strokeLinejoin="round"/><path d="M12 9.5v5" stroke="#1d1d1d" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="17.4" r="1.15" fill="#1d1d1d"/></svg></span>}</button></nav>
