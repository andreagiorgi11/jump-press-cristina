...
export default async function ConsentPage({searchParams}){
 ...
 let identity;try{identity=await requireEditor();}catch(e){if(e.status===401)redirect('/api/auth/login?returnTo='+encodeURIComponent('/editor/consent?request='+encodeURIComponent(request)));return <main className="editor-main narrow"><h1>Accesso non disponibile</h1><p>Controlla che il tuo account sia abilitato alla redazione.</p></main>;}
