...
export default function EditorApp({ready}){
 ...
 async function showAsset(assetId){setError('');try{const r=await fetch('/api/editor',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'asset',assetId})});const data=await r.json();if(!r.ok)throw Error(data.error||'Ritaglio non disponibile');window.dispatchEvent(new CustomEvent('jump-open-clip',{detail:{url:data.url}}));}catch(e){setError(e.message);}}
 ...
 const editorActions=['editor','publisher'].includes(role)?{
  onDelete:current&&!current.publishedVersions?.length?async()=>{await mutate('delete',current);setCurrent(null);setNotice('Bozza spostata nel cestino.');history.replaceState(null,'','/editor');}:undefined,
  ...
  onRecover:async row=>{const d=await mutate('recover',row);setCurrent(d);setNotice('Bozza ripristinata.');history.replaceState(null,'','/editor?draft='+d.id);},
 ...
 return <><AppControls editorActions={editorActions} onConfirm={current&&role==='publisher'&&!current.publishedVersions?.includes(current.version)?confirmDraft:undefined} confirmBusy={busy} confirmDate={current?.body.date} confirmVersion={current?.version}/><main>{notice&&<p role="status" className="editor-notice">{notice}</p>}{error&&<p className="editor-error" role="alert">{error}</p>}{current?<EditionView body={current.body} preview={!current.publishedVersions?.includes(current.version)} onClip={showAsset} privateClips/>:<section className="ranking"><h1 style={{fontSize:32,color:'#111'}}>Nessuna bozza disponibile</h1><p>La rassegna preparata tramite GPT apparirà qui.</p></section>}</main></>;
}
