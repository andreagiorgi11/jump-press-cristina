...
export default function EditorApp({ready}){
 ...
 const Edition=process.env.NEXT_PUBLIC_JUMP_APPROVAL_LIVE==='1'?ApprovalEdition:EditionView;
 ...
 return <>{readerPreview?<div className="appcontrols reader-preview-controls" aria-label="Anteprima lettore"><span>Anteprima lettore</span><button type="button" onClick={()=>setReaderPreview(false)}>← Torna alla modifica</button></div>:<AppControls onReaderPreview={current&&editorActions&&!editing?()=>setReaderPreview(true):undefined} editorActions={editorActions} onConfirm={current&&!editing&&role==='publisher'&&(!current.publishedVersions?.length||current.withdrawnAt)?confirmDraft:undefined} confirmBusy={busy} isRevision={!!current?.publishedVersions?.length} confirmDate={current?.body.date} confirmVersion={current?.version}/>}<Frame>{!readerPreview&&notice&&<p role="status" className="editor-notice">{notice}</p>}{error&&<p className="editor-error" role="alert">{error}</p>}{!readerPreview&&current&&editorActions&&current.publishedVersions?.length>0&&!current.withdrawnAt&&<div className="revision-entry"><WithdrawEdition id={current.id} version={current.version} date={current.body.date} icon={false} onWithdraw={d=>{setCurrent(d);setNotice('Rassegna ritirata dal sito e riportata in bozza.');}}/></div>}{current&&editing&&<RevisionEditor key={current.id+':'+editing.section+':'+(editing.articleId||'')} draft={current} selection={editing} onClose={()=>setEditing(null)} onSaved={d=>{setCurrent(d);setNotice('Correzione salvata in bozza.');}}/>}{current?<Edition draftId={current.id} version={current.version} onEdit={!readerPreview&&editorActions&&(!current.publishedVersions?.length||current.withdrawnAt)?setEditing:undefined} body={current.body} preview={!readerPreview&&!current.publishedVersions?.includes(current.version)} onClip={showAsset} privateClips editorStatus={!readerPreview}/>:!current&&<section className="ranking"><h1 style={{fontSize:32,color:'#111'}}>Nessuna bozza disponibile</h1><p>La rassegna preparata tramite GPT apparirà qui.</p></section>}</Frame></>;
}
