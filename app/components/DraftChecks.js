...
export default function DraftChecks({body}){
 ...
 return <aside className="article-checks" aria-label="Attenzioni sulla bozza"><p><strong>Bozza salvata con elementi da verificare.</strong> {pending>0&&`${pending} articoli: verifica PDF in attesa. `}{pdf>0&&`${pdf} articoli: PDF da verificare. `}{[...new Set(labels)].map(label=>`${labels.filter(x=>x===label).length} articoli: ${label.toLowerCase()}. `)}{covers>0&&`${covers} copertine: PDF da verificare. `}I dettagli sono sotto i singoli articoli.</p></aside>;
}
