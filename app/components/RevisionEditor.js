...
export default function RevisionEditor({draft,selection,onSaved,onClose}){
 ...
 function remove(){if(busy||!a||body.articles.length<2)return;if(!window.confirm('Eliminare dalla bozza l’articolo «'+a.title+'»? Il conteggio degli articoli si aggiorna.'))return;save(null,{...body,articles:body.articles.filter(x=>x.id!==a.id)});}
 ...
 <div className="article-placement-row"><label>Categoria<select value={a.category===NATIONAL_TEAM?NATIONAL_TEAM:summaryCategory(a)} onChange={e=>article('category',e.target.value)}>{categories.map(c=><option key={c} value={c}>{c}</option>)}</select></label><label>Posizione nella sezione<select value={sectionIds.indexOf(a.id)+1} onChange={e=>setBody(b=>moveArticleInSection(b,a.id,Number(e.target.value),summaryView))}>{sectionIds.map((id,index)=><option key={id} value={index+1}>{index+1} di {sectionIds.length}</option>)}</select></label></div>
