...
const html='<!doctype html><html lang="it"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Credenziali Jump Press</title><style>body{font:17px system-ui;background:#f4f4f1;color:#202020;margin:32px auto;max-width:520px;padding:20px}label{display:block;margin:20px 0}input,button{box-sizing:border-box;width:100%;padding:14px;font:inherit}button{background:#141414;color:white;border:0;border-radius:8px}</style><h1>Accessi Jump Press</h1><p>Scegli due password distinte, da 14 a 256 caratteri. Le password restano sul computer: verranno salvate solo le impronte protette da trasferire nei segreti Vercel.</p><form method="post" action="/save"><input type="hidden" name="csrf" value="'+csrf+'"><h2>Utente andrea</h2><label>Password<input type="password" name="andrea" autocomplete="new-password" minlength="14" maxlength="256" required></label><label>Ripeti password<input type="password" name="andreaConfirm" autocomplete="new-password" minlength="14" maxlength="256" required></label><h2>Utente cristina</h2><label>Password<input type="password" name="cristina" autocomplete="new-password" minlength="14" maxlength="256" required></label><label>Ripeti password<input type="password" name="cristinaConfirm" autocomplete="new-password" minlength="14" maxlength="256" required></label><button>Prepara gli accessi</button></form></html>';
const server=createServer(async(req,res)=>{
 ...
 const a=fields.get('andrea'),c=fields.get('cristina');if(a!==fields.get('andreaConfirm')||c!==fields.get('cristinaConfirm')||a===c)return reply(400,'Le conferme devono corrispondere e le due password devono essere diverse. Torna indietro e riprova.');
 const users=[{id:'268693493',username:'andrea',role:'publisher',passwordHash:await passwordHash(a)},{id:'319848993',username:'cristina',role:'publisher',passwordHash:await passwordHash(c)}];
